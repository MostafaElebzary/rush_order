...
@section('breadcrumb')
    <h1 class="d-flex flex-column text-dark fw-bolder fs-3 mb-0"> صور المقالات | {{$data->title}}</h1>
@endsection
    ...
    <script type="text/javascript">
        $(function () {
            var table = $('#data_table').DataTable({
                ...
                ajax: {
                    url: '{{ route('image.post.datatable.data',$data->id) }}',
                    data: {}
                },
                columns: [
                    {data: 'checkbox', name: 'checkbox', "searchable": false, "orderable": false},
                    {data: 'post', name: 'post', "searchable": true, "orderable": true},
            ...
            $.ajax({
                url: "{{ URL::to('admin/add-post-image-button/'.$data->id)}}",
